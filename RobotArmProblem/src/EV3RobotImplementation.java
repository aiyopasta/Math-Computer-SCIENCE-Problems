...
public class EV3RobotImplementation extends JPanel implements Runnable{
	...
	public double evalFunction(double x) {
		//ENTER FUNCTION HERE
		double y = Math.abs(x); //trig functions are weird...
		return y;
	}
	...
	public void run() {
		...
		for (int i=1;i>=-1;i-=2) {
			...
			while (iPad.contains(new Point2D.Double(drawX, SCALE*arm.getPoint().getY())) && !arm.breakPoint) {
				...
				try {
					t.sleep(20); //see if code waits for physical motor to move. if it doesn't adjust sleep time accordingly.
	...
	public static void main(String[] args) {
		EV3RobotImplementation app = new EV3RobotImplementation();
	}
}